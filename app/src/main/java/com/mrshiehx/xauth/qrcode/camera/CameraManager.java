...
public final class CameraManager {
    ...
    public synchronized void requestPreviewFrame(Handler handler, int message) {
        ...
        if (theCamera != null && previewing) {
            ...
            theCamera.getParameters().setPreviewFormat(ImageFormat.JPEG);
            theCamera.setOneShotPreviewCallback(previewCallback);
			/*theCamera.setOneShotPreviewCallback(new Camera.PreviewCallback() {
				@Override
				public void onPreviewFrame(byte[] data, Camera camera) {
					try{
						FileUtils.bytes2File(data,new File("/sdcard/MSChatRoom/"+System.currentTimeMillis()+",341.png"));
					}catch (IOException e){
						e.printStackTrace();
					}
				}
...
//					MIN_FRAME_HEIGHT, MAX_FRAME_HEIGHT);
            ...
            int height = /*findDesiredDimensionInRange(screenResolution.y,
					MIN_FRAME_HEIGHT, MAX_FRAME_HEIGHT)*4/5*/width;
    ...
    public PlanarYUVLuminanceSource buildLuminanceSource(byte[] data,
		...
		/*Toast.makeText(context, String.format("%d,%d,%d,%d,%d,%d",width, height, rect.left,
				rect.top, rect.width(), rect.height()), Toast.LENGTH_SHORT).show();*/
